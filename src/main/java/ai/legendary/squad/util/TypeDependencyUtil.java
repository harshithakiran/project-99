package ai.legendary.squad.util;
...
public class TypeDependencyUtil {
	...
	private static final Map<String, List<TypedDependency>> map = new HashMap<String, List<TypedDependency>>();

	private boolean DebugTypeDependencyUtil = true;

	public static PennTreebankLanguagePack getPennTreebankLanguagePack() {
		return languagePack;
	}
	...
	public static GrammaticalStructureFactory getGrammaticalStructureFactory() {
	...
	private static String getRelation(final List<TypedDependency> dependencies) {
		...
		// else if (dependency.reln().toString().contains("cop") &&
		// dependency.gov().word().equals(relation))
		// relation = dependency.dep().word();
	...
	private static String getObject(final List<TypedDependency> dependencies) {
		...
		for (int i = dependencies.size() - 1; i >= 0; i--) {
			...
			else if (dependency.reln().toString().contains("compound")
					&& dependency.gov().word().equals(rootDirectObject))
	...
	public static Map<String, String> getAbbreviations(final String text) {
		...
		for (int i = dependencies.size() - 1; i >= 0; i--) {
			...
			} else if (key != null && dependency.reln().toString().contains("compound")
					&& dependency.gov().word().contains(rootValue)) {
	...
	public static class TypeDependencyData {
		...
		public String toString() {
			return "TypeDependencyData{" + "subject='" + subject + '\'' + ", relation='" + relation + '\''
					+ ", object='" + object + '\'' + '}';
		}
	}
}
